    ...
    with torch.no_grad():
        if test_10crop:
            ...
            for i in range(len(loader['test'])):
                print("it_test: {:05d} / {:05d}".format(i, len(loader['test'])))
                data = iter_test.next()
                inputs = data[0]
                labels = data[1]
                inputs = inputs.cuda()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--s_dset_path', type=str, default=osp.join(osp.dirname(__file__),'../data/office/amazon_list.txt'), help="The source dataset path list")
    parser.add_argument('--t_dset_path', type=str, default=osp.join(osp.dirname(__file__),'../data/office/webcam_list.txt'), help="The target dataset path list")
    ...
    # config["data"] = {"source":{"list_path":args.s_dset_path, "batch_size":20}, \
